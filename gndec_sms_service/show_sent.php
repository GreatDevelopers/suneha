...
echo "<center><IMG SRC='images/title.png' WIDTH=570 HEIGHT=90 ALT='' BORDER=0></IMG></center>";
	...
	$sql = "SELECT receiver, mobile, msgdata, time,msgid FROM report WHERE sender = '".$username."' ORDER BY report.id DESC";
...
while($row = mysql_fetch_assoc($result))
  {
		$reslt = mysql_query("SELECT status FROM tdlr WHERE msgid='".$row['msgid']."'") or die(mysql_error());
		$status = mysql_fetch_assoc($reslt);
		if($row['msgid']=='') {
			$status_f = 'Unknown';
		}
