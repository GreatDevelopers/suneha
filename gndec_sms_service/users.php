		...
		case "adduser":
			...
			if ((!empty($newuserName)) && (isAlphaNumeric($newuserName))) {
				if ($_POST['newuserPass'] == $_POST['newuserConfirmPass']) {
					...
					$dupdetails = mysql_num_rows(mysql_query("SELECT username FROM users WHERE mobile='".$newuserMobile."' OR email='".$newuserEmail."'"));
					if($dupdetails!=0) {
						$actionMsg = $lang['ERR_DUP_EMAIL_MOBILE'];
					}
					else {
